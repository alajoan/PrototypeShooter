...
using UnityEngine;
using System;
public class scriptSpawnFlameRed : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (count > 3f)
        {
            ...
            GameObject parentToFlame = TrashMan.spawn("Flame_Red_Attack1", positionToSpawn,muzzle.transform.rotation);
        ...
        /*if (!particleSys.IsAlive())
        {
            ...
            GameObject parentToFlame = TrashMan.spawn("Flame_Red_Attack1", positionToSpawn);
            parentToFlame.transform.parent = gameObject.transform.parent;
            positionToSpawn = new Vector3(0, 0, 0);
            TrashMan.despawn(gameObject);
